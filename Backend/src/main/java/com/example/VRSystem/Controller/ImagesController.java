...
import com.example.VRSystem.Model.AdminImages;

import com.example.VRSystem.Model.UserImages;
...
public class ImagesController {
	 ...
	 * @Transactional
	 * 
	 * @PostMapping("/doctor/image/upload/{id}") public String
	 * uplaodDoctorImage(@RequestParam("imagefile") MultipartFile
	 * file, @PathVariable("id") Long id) throws IOException {
	 * System.out.println("Original Image Byte Size - " + file.getBytes().length);
	 * if(doctorImages_repository.isImageExistByDoctorID(id) == 1) {
	 * doctorImages_repository.updateDoctorImage(file.getOriginalFilename(),
	 * file.getContentType(), compressBytes(file.getBytes()), id); Long imageID =
	 * doctorImages_repository.findImageIDByDoctorID(id);
	 * doctor_repository.updateImageIDByDoctorID(imageID, id); return
	 * "Image updated successfully"; }
	 * doctorImages_repository.insertDoctorImage(file.getOriginalFilename(),
	 * file.getContentType(), compressBytes(file.getBytes()), id); Long imageID =
	 * doctorImages_repository.findImageIDByDoctorID(id);
	 * doctor_repository.updateImageIDByDoctorID(imageID, id); return
	 ...
	 * @Transactional
	 * 
	 * @GetMapping(path = { "/doctor/image/get/{id}" }) public DoctorImages
	 * getDoctorImage(@PathVariable("id") Long id) throws IOException {
	 * if(doctorImages_repository.isImageExistByDoctorID(id) == 1) { final
	 * Optional<DoctorImages> retrievedImage =
	 * doctorImages_repository.findByDoctorDoctorID(id); DoctorImages img = new
	 * DoctorImages(retrievedImage.get().getName(), retrievedImage.get().getType(),
	 * decompressBytes(retrievedImage.get().getpic_byte()));
	 * img.setDoctor(retrievedImage.get().getDoctor()); return img; } return null; }
	 */
	 ...
	 * @Transactional
	 * 
	 * @GetMapping(path = { "/doctor/{department}/image" }) public
	 * List<DoctorImages> getDoctorImages(@PathVariable("department") String
	 * department) throws IOException {
	 * if(doctorImages_repository.isImageExistByDepartment(department) > 0) { return
	 * doctorImages_repository.findByDoctorDepartment(department); } return null; }
	 */
